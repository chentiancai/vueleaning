<template lang="html">
     <div class="container"> 
        <HomeHeader></HomeHeader> <!-- 展示引入的header组件 --> 
        <div class="content">
            <ul class="cont-ul"> 
                <!-- list组件展示区，并用v-for来将数据遍历，:xx="xxx" 是用来给子组件传递数据的 --> 
                <list v-for="item in items" :key="item_key" :price="item.price" :title="item.title" :img="item.img">
                </list> 
            </ul> 
        </div> 
        <CommonFooter></CommonFooter>  <!-- 展示引入的footer组件 --> 
    </div>
</template>
